...
/**
 * The AcceptEurekaApplicationNameServiceImpl.
 * @author chaocoffee
 */
@Service
@Slf4j
public class AcceptEurekaApplicationNameServiceImpl<T> implements AcceptApplicationNameService {
    ...
    public <T> List<T> acceptAppNameList(List<T> apps) {
        ...
        if (updateTime == null) {
            ...
            getEurekaAppList().forEach(appName -> CACHE_MAP.put(appName, appName));
        } else {
            long dValue = System.currentTimeMillis() - (Long) updateTime;
            if (dValue > Long.valueOf(appIntervalTime) * COUNT) {
